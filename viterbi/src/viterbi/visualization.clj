...
(defn escapePunctChar
  ...
  (cond
   ...
   (includes? text "</") (replace text #"</" "ltS")
...
(defn createNode
  ...
  [word pos num]
   [:dali/align {:axis :center}
     [:rect {:id (keyword (str (escapePunctChar word) "|" (escapePunctChar pos))) :stroke {:width 3} :fill :white} :_ [(* (count (str word "|" pos "(" num ")")) 9) 40]]
  ...
  (defn connect2Columns
    ...
    (if (empty? firstColumn)
      []
    (apply vector (concat (connect2Columns (into [] (rest firstColumn)) secColumn bestSeq)
     (apply vector
       (map (fn [x]
              (if (and (some (fn [y2] (and (= y2 (str (get x 0)"|"(get x 1))) (not (= y2 nil)))) bestSeq)
                      (some (fn [y1] (= y1 (str (get (first firstColumn) 0) "|" (get (first firstColumn)1)))) bestSeq))
                  [:dali/connect
                  {:from (keyword (str (escapePunctChar (get (first firstColumn) 0))"|" (escapePunctChar (get (first firstColumn) 1))))
                  :to (keyword (str (escapePunctChar (get x 0)) "|" (escapePunctChar (get x 1)))) :stroke :red :stroke-width 2.5 :dali/marker-end {:id :sharp :style "fill: red;"}}]
                  [:dali/connect
                  {:from (keyword (str (escapePunctChar (get (first firstColumn) 0))"|" (escapePunctChar (get (first firstColumn) 1))))
                  :to (keyword (str (escapePunctChar (get x 0)) "|" (escapePunctChar (get x 1)))) :stroke :black :stroke-width 2.5 :dali/marker-end :sharp}]))
        secColumn))))))
